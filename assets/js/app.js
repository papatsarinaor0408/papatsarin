...
miniKpis.innerHTML =     <div class="mini-kpi-card">
...
document.getElementById('budget-insight-panel').innerHTML =     <div class="insight-panel">
      <div>
...
const data = getFiltered();
root.innerHTML =     <div class="filter-count" style="margin-bottom:10px;">พบ ${fmtNum(data.length)} แผน จากทั้งหมด ${fmtNum(STATE.records.length)} แผน</div>
...
root.innerHTML =     <div class="filter-count" style="margin-bottom:10px;">สรุปตามหน่วยงานระดับ <b>${ORG_LEVELS.find((o) => o.key === orgLevel).label}</b> — คลิกแถวเพื่อดูรายละเอียดประเภทแผน</div>
    <div class="table-wrap">
      <table class="data-table">
        <thead><tr>
...
${Object.keys(row.byDeliveryType).length ?               <div style="font-size:12px;font-weight:600;color:var(--text-muted);margin-bottom:6px;text-transform:uppercase;">แยกตามประเภทการอบรม</div>
...
root.innerHTML =     <div class="filter-bar" style="margin-bottom:14px;">
...
const history = r.reviewStatus !== 'pending' ?     <div class="review-history">
...
const considerationSection = isCentral ?     <div class="section-heading">ข้อมูลสำหรับพิจารณา</div>
...
area.innerHTML =     <label for="decision-note" style="display:block;font-size:11px;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:.02em;margin-bottom:6px;">หมายเหตุของผู้พิจารณา</label>
...
bar.innerHTML =     <div class="filter-field">
