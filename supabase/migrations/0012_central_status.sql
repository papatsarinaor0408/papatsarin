-- New review-decision status: "รวบรวมให้ อศค. ดำเนินการ" (routed to อศค. to
-- handle centrally — not a decision the plant executive makes themselves).
...
alter table public.decisions add constraint decisions_decision_check
  check (decision in ('pending','approved','revise','rejected','central'));
...
begin
  ...
  if p_decision not in ('pending','approved','revise','rejected','central') then
  ...
  values (v_uid, v_employee_id, v_role,
    case p_decision when 'approved' then 'Approved' when 'revise' then 'Revise Requested'
                     when 'rejected' then 'Rejected' when 'central' then 'Routed to อศค.' else 'Reverted to Pending' end,
